...
save(results_repeat_nmf_200_10, file = "~/rnaGinesis/data/temp_experiment2_results_repeat_200_10.RData")
...
save(results_vanilla_nmf_200_10, file = "~/rnaGinesis/data/temp_experiment2_results_vanilla_200_10.RData")
...
save(results_vanilla_nmf_1_10, file = "~/rnaGinesis/data/temp_experiment2_results_vanilla_1_10.RData")
...
first_true <- list()
first_true$trueW <- experiment2$trueW
first_true$trueH <- experiment2$trueH
# make sum to 1
...
p4 <-ggplot2::ggplot(boxplot_wcosd, ggplot2::aes(x=level_order, y=W_COSDIST, color=method)) +
  ggplot2::geom_boxplot() + ylim(0,0.12) + 
  ggplot2::theme(
  ...
  ylab("W COSDIST") + ggtitle("ResampledSingleCell")
print(p4)
```
...
p4 <-ggplot2::ggplot(boxplot_rmse, ggplot2::aes(x=level_order, y=H_RMSE, color=method)) +
  ...
  axis.title.y = ggplot2::element_text(size = 20),
  axis.title.x = element_blank(), axis.text.x = ggplot2::element_text(size = 14, face="bold"), axis.text.y = ggplot2::element_text(size = 14, face="bold"), legend.position = "none", legend.title= element_blank()) + 
  ylab("H RMSE") + ggtitle("ResampledSingleCell")
print(p4)
```
...
```{r}
load("~/rnaGinesis/data/alisa_exp2.RData")
# took 1.2 hrs for 50_20_300_10; vanilla for 30 iterations in 10 minutes
dataset <- experiment2$ex #[1:5000,]
rank = 3
.options = "p4"
start_time <- Sys.time()
nrun = 300
results_repeat_50_20_300_10 <- list()
for (i in 1:10) {
  res <- repeatability_main_function(dataset, nrun, rank, i)
  results_repeat_50_20_300_10[[i]] <- res
  print(i)
}
#nrun = 300
#for (i in 21:30) {
...
save(results_repeat_50_20_300_10, file = "~/rnaGinesis/data/temp_experiment2_results_repeat_50_20_300_10.RData")
...
save(results_vanilla_nmf_30, file = "~/rnaGinesis/data/temp_experiment2_results_vanilla_10.RData")
```
...
first_true$trueH <- experiment2$trueH
first_true$trueW <- experiment2$trueW
#first_true$trueW <- lapply(experiment2$trueW, function(X) X[1:5000,])
#first_true$trueH <- results_vanilla_nmf_30[[i]]@fit@H
#first_true$trueW <- results_vanilla_nmf_30[[i]]@fit@W
...
p <- p + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0,1.0) 
...
p2 <- p2 + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0.05,0.075) 
...
first_true$trueW <- experiment2$trueW
#first_true$trueW <- lapply(experiment2$trueW, function(X) X[1:5000,])
#first_true$trueH <- results_repeat_50_20_300_10[[i]]@fit@H
#first_true$trueW <- results_repeat_50_20_300_10[[i]]@fit@W
#first_true$trueW <- lapply(seq_len(ncol(first_true$trueH)), function(X) first_true$trueW)
# make sum to 1
first_true$trueH <- apply(first_true$trueH, 2, FUN = sum_to_1)
...
for (i in 1:10) {
  exp <- list()
  exp$resultW <- results_repeat_50_20_300_10[[i]]@fit@W
  exp$resultH <- results_repeat_50_20_300_10[[i]]@fit@H
  exp$resultW <- lapply(seq_len(ncol(exp$resultH)), function(X) exp$resultW)
  # make sum to 1
  exp$resultH <- apply(exp$resultH, 2, FUN = sum_to_1)
  print("W")
...
m_W.COSDIST <- t(m_W.COSDIST[1,])
m_H.RMSE <- t(m_H.RMSE[1,])
...
p <- p + ggplot2::geom_violin()
p <- p + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0.02, 0.1) #(0.35,0.50) 
p <- p + ggplot2::labs(title="Violin Plots of Root Mean Squared Error (RMSE) for Proportions",
                       x = "Deconvolution Method",
                       y = "H_RMSE")
p <- p + ggplot2::theme(axis.title.x = ggplot2::element_text(size = 40)) 
...
p2 <- p2 + ggplot2::geom_violin()
p2 <- p2 + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0.044, 0.046) #(0.055,0.058) 
p2 <- p2 + ggplot2::labs(title="Violin Plots of Cosine Distance (COSDIST) for Gene Expression",x = "Deconvolution Method", y = "W_COSDIST") + ggplot2::theme(axis.title.x = ggplot2::element_text(size = 40)) + ggplot2::theme(axis.title.y = ggplot2::element_text(size = 40)) + ggplot2::theme(axis.text=ggplot2::element_text(size=30))
print(p2)
```
...
p4 <-ggplot2::ggplot(boxplot_wcosd, ggplot2::aes(x=method, y=W_COSDIST, color=method)) +
  ggplot2::geom_boxplot() + ggplot2::labs(title="SingleCellMuraro Dataset: Cosine Distance for Gene Expression")
print(p4)
```
