...
##############
# do the fastanmf run instead
fastres <- fastanmf(mixture, rank=3, nrun=200)
w = basis(fastres)
h = coef(fastres)
...
w <- lapply(seq_len(ncol(h)), function(X) w)
# Saves results to list
Dataset_ResampledSingleCell.NMF <- list()
Dataset_ResampledSingleCell.NMF$resultW <- w
Dataset_ResampledSingleCell.NMF$resultH <- h
...
improvedW <- foreach (sample_n=1:ncol(experiment_to_use$trueH)) %dopar% {
  #sample_n = 1
  sample_w <- vector()
  for (gene_n in 1:nrow(experiment_to_use$ex)) {
    compartments_n <- nrow(experiment_to_use$trueH)
...
all(improvedW[[1]] > 0.0)
save(improvedW, file="data/improvedW_fasta200_ResampledSingleCell.RData")
...
#### now
```{r}
# load("data/improvedW_fasta200_ResampledSingleCell.RData")

# Evaluate the Error between true and nmf, then true and newly recomputed -

sample_avg_nmf_error_zeros <- list()
sample_avg_nmf_error_nonzero <- list()
sample_avg_new_error_zeros <- list()
sample_avg_new_error_nonzero <- list()
for (sample_n in 1:ncol(experiment_to_use$trueH)) {
  nmf_error_zeros <- list()
  nmf_error_nonzero <- list()
  new_error_zeros <- list()
  new_error_nonzero <- list()
  for (gene_n in 1:nrow(experiment_to_use$ex)) {
    nmf_w <- as.vector(resultW.NMF[[sample_n]][gene_n,])
    true_w <- as.vector(trueW[[sample_n]][gene_n,])
    new_w <- improvedW[[sample_n]][gene_n,]
    ...
    for (i in 1:compartments_n) {
      if (true_w[i] == 0) {
        nmf_error_zeros <- c(nmf_error_zeros, abs(nmf_w[i]-true_w[i]))
        new_error_zeros <- c(new_error_zeros, abs(new_w[i]-true_w[i]))
      } else {
        nmf_error_nonzero <- c(nmf_error_nonzero, abs(nmf_w[i]-true_w[i])/true_w[i])
        new_error_nonzero <- c(new_error_nonzero, abs(new_w[i]-true_w[i])/true_w[i])
      } 
    }
  }   
  # do average over all genes and all compartments
  sample_avg_nmf_error_zeros[[sample_n]] <- mean(unlist(nmf_error_zeros))
  sample_avg_new_error_zeros[[sample_n]] <- mean(unlist(new_error_zeros))
  sample_avg_nmf_error_nonzero[[sample_n]] <- mean(unlist(nmf_error_nonzero))
  sample_avg_new_error_nonzero[[sample_n]] <- mean(unlist(new_error_nonzero))
  print(sample_n)
}
