...
if (is.null(rownames(dataset))) {
  rownames(dataset) <- paste0("gene",as.character(1:nrow(dataset)))
}
rank = 3
start_time <- Sys.time()
...
save(results_vanilla_nmf_1_10, file = "~/rnaGinesis/data/temp_results_vanilla_1_10.RData")
...
save(results_vanilla_nmf_200_10, file = "~/rnaGinesis/data/temp_results_vanilla_200_10.RData")
...
save(results_repeat_nmf_200_10, file = "~/rnaGinesis/data/temp_results_repeat_200_10.RData")
...
first_true <- list()
first_true$trueW <- experiment1$trueW
first_true$trueH <- experiment1$trueH
# make sum to 1
...
p4 <-ggplot2::ggplot(boxplot_wcosd, ggplot2::aes(x=level_order, y=W_COSDIST, color=method)) +
  ggplot2::geom_boxplot() + ggplot2::labs(title="Simulation Dataset: Cosine Distance for Gene Expression") + ylim(0,0.00025) + 
  ggplot2::theme(
  ...
  ylab("W COSDIST") + ggtitle("Simulation")
print(p4)
```
...
p4 <-ggplot2::ggplot(boxplot_rmse, ggplot2::aes(x=level_order, y=H_RMSE, color=method)) +
  ...
  axis.title.y = ggplot2::element_text(size = 20),
  axis.title.x = element_blank(), axis.text.x = ggplot2::element_text(size = 14, face="bold"), axis.text.y = ggplot2::element_text(size = 14, face="bold"), legend.position = "none", legend.title= element_blank()) + 
  ylab("H RMSE") + ggtitle("Simulation")
print(p4)
```
...
```{r}
#Experiment to see the reproduceability of Vanillay NMF by seeding NMF with results from a previous run
load("~/rnaGinesis/data/alisa_toy_identical_w_simulated_h.RData")
dataset <- experiment1$ex
if (is.null(rownames(dataset))) {
  rownames(dataset) <- paste0("gene",as.character(1:nrow(dataset)))
}
# order dataset with most variable genes first
d_decreasing <- dataset[order(rowVars(dataset), decreasing = TRUE),]
# get names of rest of genes, not counting the 10 most variable
all_below_1K <- rownames(d_decreasing[-(1:10),])
rank = 3
start_time <- Sys.time()
results_vanilla_nmf_30 <- list()
results_vanilla_nmf_30_seed_model <- list()
for (i in 1:30) {
  res <- NMF::nmf(dataset, rank = rank, seed = i+123456, nrun = 1, .options = "p4")
  # for the seed model, set all the rest of the less variable genes to 0.01
  # make seed 
  seed_W <- res@fit@W
  # now, set everything in W to 0.01 and in H to 0.01
  #seed_W[all_below_1K,] <- 0.01
  #seed_W[,] <- 0.01
  seed_model <- NMF::nmfModel(rank = rank, W = seed_W, H = res@fit@H)
  seed_res <- NMF::nmf(dataset, rank = rank, seed = seed_model, nrun = 1, .options = "p4")
  results_vanilla_nmf_30[[i]] <- res
  results_vanilla_nmf_30_seed_model[[i]] <- seed_res
  print(i)
}  
end_time <- Sys.time()
end_time - start_time

save(results_vanilla_nmf_30, file = "~/rnaGinesis/data/temp_results_vanilla_30.RData")
save(results_vanilla_nmf_30_seed_model, file = "~/rnaGinesis/data/temp_results_vanilla_30_seed_model.RData")
...
#load("~/rnaGinesis/data/alisa_toy_identical_full_rearrange.RData")
load("~/rnaGinesis/data/alisa_toy_identical_w_simulated_h.RData")
...
start_time <- Sys.time()
nrun = 300
results_repeat_300_50_10_10 <- list()
#results_vanilla_nmf_30_seed_model <- list()
for (i in 1:10) {
  res <- repeatability_main_function(dataset, nrun, rank, i)
  results_repeat_300_50_10_10[[i]] <- res
  
  #res <- results_repeat_300_50_10_10[[i]]
  #seed_model <- NMF::nmfModel(rank = rank, W = res@fit@W, H = res@fit@H)
  #seed_res <- NMF::nmf(dataset, rank = rank, seed = seed_model, nrun = 1, .options = "p4")
  #results_vanilla_nmf_30_seed_model[[i]] <- seed_res
  print(i)
}
...
save(results_repeat_300_50_10_10, file = "~/rnaGinesis/data/temp_results_repeat_300_50_10_10.RData")
#save(results_vanilla_nmf_30_seed_model, file = "~/rnaGinesis/data/temp_results_vanilla_30_seed_model.RData")

```

```{r}
# Test Experiment1 correlation matrix
#load("~/rnaGinesis/data/alisa_new_A_mu_exp1_no_noise_high_rearrange.RData")
load("~/rnaGinesis/data/alisa_toy_identical_full_rearrange.RData")
meanW <- experiment1$unmixed
cor = cor(meanW, meanW)
cor
cor = cor(meanW, experiment1$ex)
cor
cosdist <- evalW.COSDIST(experiment1$unmixed,experiment1$trueW[[1]] %*% experiment1$trueH[,1])
```
...
results_vanilla_nmf_30 <- list()
for (i in 1:30) {
  #rand_seed <- 123456 + i*10000 + i*100
  rand_seed <- i
  res <- NMF::nmf(dataset, rank = rank, seed = rand_seed, nrun = 3, .options = "p4")
  results_vanilla_nmf_30[[i]] <- res
  print(i)
}  
end_time <- Sys.time()
...
# get trueH and trueW from the first run
first_true <- list()
#first_true$trueW <- results_vanilla_nmf_300[[1]]@fit@W
#first_true$trueH <- results_vanilla_nmf_300[[1]]@fit@H
...
for (i in 1:20) {
  exp <- list()
  exp$resultW <- results_vanilla_nmf_30_seed_model[[i]]@fit@W
  exp$resultH <- results_vanilla_nmf_30_seed_model[[i]]@fit@H
  exp$resultW <- lapply(seq_len(ncol(exp$resultH)), function(X) exp$resultW)
  # make sum to 1
  exp$resultH <- apply(exp$resultH, 2, FUN = sum_to_1)
  # re-arrange the results
...
p <- p + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0,0.15) 
...
p2 <- p2 + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0,0.00025) 
...
# get trueH and trueW from the first run
first_true <- list()
#first_true$trueW <- results_repeat_300_50_10_10[[1]]@fit@W
#first_true$trueH <- results_repeat_300_50_10_10[[1]]@fit@H
#first_true$trueW <- lapply(seq_len(ncol(first_true$trueH)), function(X) first_true$trueW)
first_true$trueW <- experiment1$trueW
first_true$trueH <- experiment1$trueH
# make sum to 1
...
for (i in 1:20) {
  exp <- list()
  exp$resultW <- results_repeat_300_50_10_10[[i]]@fit@W
  exp$resultH <- results_repeat_300_50_10_10[[i]]@fit@H
...
# make jitter reproduceable
set.seed(1234)
#p <- ggplot2::ggplot(data = m_mydf, ggplot2::aes(y = H_RMSE, x= method, fill = method))
p <- ggplot2::ggplot() + ggplot2::geom_point(data = m_mydf, ggplot2::aes(y = H_RMSE, x= method, fill = method), color="blue")
p <- p + ggplot2::geom_point(data = mydf, ggplot2::aes(y = H_RMSE, x= method, fill = method), color="black")
p <- p + ggplot2::geom_violin()
p <- p + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0,0.15) 
p <- p + ggplot2::labs(title="Violin Plots of Root Mean Squared Error (RMSE) for Proportions",
                       x = "Deconvolution Method",
...
set.seed(1234)
#p2 <- ggplot2::ggplot(data = mydf2, ggplot2::aes(y = W_COSDIST, x= method, fill = method))
p2 <- ggplot2::ggplot() + ggplot2::geom_point(data = m_mydf2, ggplot2::aes(y = W_COSDIST, x= method, fill = method), color = "blue")
p2 <- p2 + ggplot2::geom_point(data = mydf2, ggplot2::aes(y = W_COSDIST, x= method, fill = method), color = "black")
p2 <- p2 + ggplot2::geom_violin()
p2 <- p2 + ggplot2::geom_jitter(width = 0.1, height = 0) + ggplot2::ylim(0.0, 0.00025) 
p2 <- p2 + ggplot2::labs(title="Violin Plots of Cosine Distance (COSDIST) for Gene Expression",x = "Deconvolution Method", y = "W_COSDIST") + ggplot2::theme(axis.title.x = ggplot2::element_text(size = 40)) + ggplot2::theme(axis.title.y = ggplot2::element_text(size = 40)) + ggplot2::theme(axis.text=ggplot2::element_text(size=30))
print(p2)
```
...
p4 <-ggplot2::ggplot(boxplot_wcosd, ggplot2::aes(x=method, y=W_COSDIST, color=method)) +
  ggplot2::geom_boxplot() + ggplot2::labs(title="Simulation Dataset: Cosine Distance for Gene Expression")
print(p4)
```
