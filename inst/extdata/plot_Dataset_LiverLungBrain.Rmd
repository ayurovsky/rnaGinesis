---
title: "plot_Dataset_LiverLungBrain"
...
### Makes plots to evaluate deconvolution methods on brain, liver, lung mixture data

#### Obtain trueH, trueW and resultH, resultW from each deconvolution method
```{r}
# Obtains trueW, trueH from .RData
load("~/rnaGinesis/data/Dataset_LiverLungBrain.RData")
Dataset_LiverLungBrain$trueH <- t(Dataset_LiverLungBrain$trueH)
Dataset_LiverLungBrain$trueW <- as.matrix(Dataset_LiverLungBrain$trueW)
# make a list of identical Ws because rearrange expects them...
Dataset_LiverLungBrain$trueW <- lapply(seq_len(ncol(Dataset_LiverLungBrain$trueH)), function(X) Dataset_LiverLungBrain$trueW)

trueW <- Dataset_LiverLungBrain$trueW
trueH <- Dataset_LiverLungBrain$trueH

# Obtains resultW, resultH from NMF and rearranges them to match trueW, trueH ordering
load("~/rnaGinesis/data/Dataset_LiverLungBrain.NMF.result.RData")

Dataset_LiverLungBrain.NMF <- rearrange(Dataset_LiverLungBrain.NMF, Dataset_LiverLungBrain)

resultW.NMF <- Dataset_LiverLungBrain.NMF[[1]]
resultH.NMF <- Dataset_LiverLungBrain.NMF[[2]]
...
```{r}
# melts W.COSDIST into dataframe with two variable and W.COSDIST across all samples for each deconvolution method 
