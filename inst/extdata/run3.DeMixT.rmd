...
# pick top n genes
top_n <- head(order(diff, decreasing = T), n = 100)
...
proc.time() - ptm
exp3.DEMIXT <- list()
exp3.DEMIXT$resultH <- res$pi
exp3.DEMIXT$resultW <- list()
for (i in 1:42) {
  exp3.DEMIXT$resultW[[i]] <- as.matrix(cbind(res$ExprN1[,i], res$ExprN2[,i], res$ExprT[,i]))
}
# get the indeces of the genes actually returned/used
install.packages("tidyverse")
library(stringr)
genes_used <- as.integer(word(as.character(res$gene.name),2))
exp3.DEMIXT$top_n <- top_n[genes_used]
save(exp3.DEMIXT, file = "~/rnaGinesis/data/exp3_100_DEMIXT.result.RData")
```
