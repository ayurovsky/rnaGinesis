...
load("~/rnaGinesis/data/exp2_100_DEMIXT.result.RData")

experiment2$trueW <- lapply(experiment2$trueW, function(X) X[exp2.DEMIXT$top_n,])
experiment2$ex <- experiment2$ex[exp2.DEMIXT$top_n,]
...
exp2.NMF[[1]] <- lapply(exp2.NMF[[1]], function(X) X[exp2.DEMIXT$top_n,])
...
#load("~/rnaGinesis/data/alisa_exp2.CIBERSORT.result.RData")
...
W.COSDIST <- makeW_COSDIST(trueW, resultW.NMF, resultW.DEMIXT, "NFM", "DEMIXT")
